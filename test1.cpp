...
void D2_PUTCIRCLES(struct circle Result[], int puted, double nowR) {//第一个圆预设已放入puted
	int i, j, k, flag = 0;
	...
	for (i = 0; i < MAX_POINTS_NUMS; i++) {
		...
		tmpx = tmpy = 0;
		for (j = 0; j < TIMES; j++) {//迭代一百次,梯度法，提高精度
		...
		//tmpD = Culculate_D(Result, puted, nowR, tmpX, tmpY);//计算精确后的D
		for (k = 0; k < puted; k++) {
			minResult = pow(Circles_Results[k].x - tmpX, 2) + pow(Circles_Results[k].y - tmpY, 2);
			if (nowR + Result[k].radius < pow(minResult, 0.5))
				continue;
		...
		if (flag&&tmpD<=0.0001) {
			...
			//puted++;
			D2_PUTCIRCLES(Result, puted + 1, nowR);//仍在该层继续。
			break;
		}
		else
			continue;
